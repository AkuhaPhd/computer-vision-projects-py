        ...
        if self.results.multi_face_landmarks:
            for faceLms in self.results.multi_face_landmarks:
                ...
                for idx, lm in enumerate(faceLms.landmark):
                    ...
                    cv2.putText(img, str(idx), (x, y), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 1)
                    face.append([x, y])
                faces.append(face)
        return img, faces
...
def main():
    cap = cv2.VideoCapture("Y2Mateis.mp4")
    pTime = 0
    ...
    while True:
        ...
        fps = 1 / (cTime - pTime)
        pTime = cTime
        cv2.putText(img, f"FFS: {int(fps)}", (20, 70), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
